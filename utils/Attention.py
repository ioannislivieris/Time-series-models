...
class SelfAttention(Layer):
    ...
    def __init__(self, size, num_hops=8, use_penalization=True,
                 penalty_coefficient=0.1, model_api='sequential', **kwargs):
    ...
    def build(self, input_shape):
        ...
        self.W1 = self.add_weight(name='W1',
                                  shape=(self.size, input_shape[2].value),                                # (size, H)
...
# from tensorflow.keras.layers import Layer, Multiply, Lambda, Dot, Concatenate, RepeatVector
...
#                  score_function='dot', model_api='sequential', **kwargs):
...
#             self.W_a = Dense(name='D3', units=self.hidden_dim, use_bias=False)
...
#                 aligned_position = Activation('tanh', name='attention_weight1')(aligned_position)                             # (B, 1, H)
#                 aligned_position = self.v_p(aligned_position)                                       # (B, 1, 1)
#                 aligned_position = Activation('sigmoid', name='attention_weight2')(aligned_position)# (B, 1, 1)
...
#                 aligned_position = Activation('tanh', name='attention_weight3')(aligned_position)   # (B, S, H)
#                 aligned_position = self.v_p(aligned_position)                                       # (B, S, 1)
#                 aligned_position = Activation('sigmoid', name='attention_weight4')(aligned_position)                          # (B, S, 1)
...
#                 aligned_position = Multiply(name='MultiplyLayer1')([aligned_position, onehot_vector])                    # (B, S)
#                 aligned_position = tf.expand_dims(aligned_position, axis=-1)                        # (B, S, 1)
#                 initial_source_hidden_states = source_hidden_states                                 # (B, S, 1)
#                 source_hidden_states = Multiply(name='MultiplyLayer2')([source_hidden_states, aligned_position])         # (B, S*=S(D), H)
...
#             attention_score = Dot(name='Dot1', axes=[2, 2])([source_hidden_states, target_hidden_state])         # (B, S*, 1)
...
#             attention_score = Dot(name='Dot2', axes=[2, 2])([weighted_hidden_states, target_hidden_state])       # (B, S*, 1)

#         elif self.score_function == 'location':                                                     # Location-based Score Function
#             weighted_target_state = self.W_a(target_hidden_state)                                   # (B, 1, H)
#             attention_score = Activation('softmax', name='attention_weight5')(weighted_target_state)                          # (B, 1, H)
...
#             weighted_sum = Activation('tanh', name='attention_weight6')(weighted_sum)                                         # (B, S*, H)
#             attention_score = self.v_a(weighted_sum)                                                # (B, S*, 1)

#         # Compute attention weights
#         attention_weights = Activation('softmax', name='attention_weight7')(attention_score)                                  # (B, S*, 1)
...
#                 gaussian_factor = Concatenate(name='Concatenate1', axis=1)([gaussian_factor, gaussian_estimation(i)])    # (B, S*, 1)
