...
def create_dataset(dataset, look_back=1, Horizon=1, SeriesName =''):
    ...
    for i in range(dataset.shape[0] +1  - look_back - Horizon):
        ...
        dataY.append( dataset.to_numpy()[i + look_back : i + look_back + Horizon] )
        
        
    return ( np.array(dataX), np.array(dataY).squeeze(-1) )
...
def create_dataset_diff(dataset, look_back=1, Horizon=1, SeriesName =''):
    ...
    for i in range(dataset.shape[0] - look_back):
        ...
        dataX.append( dataset.to_numpy()[i:(i+look_back)] )        
        dataY.append( dataset.to_numpy()[i + look_back] )
